...
class IOConnection implements IOCallback {
    ...
    /**
     * Transport connected. {@link IOTransport} calls this when a connection is
     * established.
     */
    ...
    /**
     * Transport disconnected. {@link IOTransport} calls this when a connection
     * has been shut down.
     */
    ...
    public void send(SocketIO socket, IOAcknowledge ack, JSONObject json) {
        IOMessage message = new IOMessage(IOMessage.TYPE_JSON_MESSAGE, socket.getNamespace(),
                json.toString());
        synthesizeAck(message, ack);
        sendPlain(message.toString());
    }
    ...
    public void emit(SocketIO socket, String event, IOAcknowledge ack, Object... args) {
        try {
            ...
            IOMessage message = new IOMessage(IOMessage.TYPE_EVENT, socket.getNamespace(),
                    json.toString());
            synthesizeAck(message, ack);
            sendPlain(message.toString());
        } catch (JSONException e) {
